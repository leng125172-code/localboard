...
At the start of LocalBoard work, run `localboard context publish --source skill`. This updates only the current execution-context entry in the single desktop activity sticky; it must not edit the shared memo or another Codex session's entry.
