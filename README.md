...
> “个人待办只保存在本地”在这里表示“不写入 GitHub Project/API”。一旦提交并推送，它仍会成为 Git 历史的一部分，远端仓库的读者可以看到。真正私密的待办不应推送；后续可增加加密的 user-only 存储模式。
...
- `integrations/codex/skills/localboard/SKILL.md` 是可安装的 Skill 源码。
...
这是第一阶段仓库，不是已完成的 GitHub Project 全功能替代品。桌面 UI 已接入个人待办、便签、Codex 活动、Project 状态与标题回写、Issue 创建/关闭、PR 和 Actions 列表；三方冲突处理、完整字段编辑与通知中心列入下一里程碑。详见 [产品范围](docs/product-scope.md) 与 [架构](docs/architecture.md)。
